...
ssize_t show_buf_size(struct device *dev, struct device_attribute *attr,
		char *buf) {
	/* get driver data and return buf size */
//	struct pcd_dev_prv_data *drv_data = NULL;
	return sprintf(buf, "%d\n", 512);
}

ssize_t set_buf_size(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count) {
	long res;
	printk("Rx buf size is %d\n", kstrtol(buf, 10, &res));
	return count;
}
...
ssize_t set_serial_num(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count) {
	return count;
}

/* the below macro creates a struct var dev_attr_set_size */
DEVICE_ATTR(size, S_IRUGO | S_IWUSR, show_buf_size, set_buf_size);
DEVICE_ATTR(serial_num, S_IRUGO | S_IWUSR, show_serial_num, set_serial_num);
...
//	pcd_dev_pf_data = pdev->dev.platform_data;
	...
	pr_info("Cfg item 1 and 2 are %d : %d \n",
			dev_cfg[pdev->id_entry->driver_data].cfg1,
			dev_cfg[pdev->id_entry->driver_data].cfg2);
	...
	/* create device */
	pcdrv_data.dev = device_create(pcdrv_data.cl, &pdev->dev,
					pcd_dev_prv_data->dev_num,
...
#if 0
	if (sysfs_create_file(&pcdrv_data.dev->kobj, &device_attr_size.attr))
#else
	if (sysfs_create_group(&pcdrv_data.dev->kobj, &attr_grp)) {
